...
for _, lib in ipairs(tree_sitter_libraries) do
...
if not vim.env.NVIM_DISABLE_PLUGINS then
  ...
  if ok then
    ...
    local available_parsers = plugin_config.get_available()
    vim.api.nvim_create_autocmd('FileType', {
      callback = function(args)
        ...
        local installed_parsers = plugin_config.get_installed 'parsers'
        ...
        elseif vim.tbl_contains(available_parsers, language) then
          ...
          plugin_config.install(language):await(function() treesitter_try_attach(buf, language) end)
        else
    ...
    local to_installed, text_objects = pcall(require "nvim-treesitter-textobjects")
    if to_installed then
      text_objects.setup {
        move = {
          set_jumps = true,
        },
    ...
    local tsm_installed, ts_move = pcall(require, "nvim-treesitter-textobjects.move")
    if tsm_installed then
    ...
    local tsrm_installed, ts_repeat_move = pcall(require, "nvim-treesitter-textobjects.repeatable_move")
    if tsrm_installed then
    ...
    local tss_installed, ts_select = pcall(require, "nvim-treesitter-textobjects.select")
    if tss_installed then
    ...
    local tssw_installed, ts_swap = pcall(require, "nvim-treesitter-textobjects.swap")
    if tssw_installed then
